"""
Script 10 — Multi-task genre classification

Adds a song-level genre prediction head to the causal GNN (v2 config).
Training loss = chord_CE + genre_weight * genre_CE.
Only songs with a known coarse genre label contribute to genre_CE.

Genre labels come from data/genre_labels.json (fetched via MusicBrainz).
Genres with < 10 songs are merged into 'other'.

Outputs:
    results/multitask_best.pt
    results/multitask_history.json
    results/multitask_training_curves.png
    results/multitask_genre_umap.png    — UMAP colored by genre (new embeddings)
    results/multitask_genre_acc.json    — per-genre accuracy on test split

Run from project root:
    source venv/bin/activate && python scripts/10_multitask_genre.py
"""
...
GENRE_WEIGHT  = 0.5    # lambda: chord_loss + GENRE_WEIGHT * genre_loss
EPOCHS        = 60
LR            = 1e-3
BATCH_SIZE    = 16
MIN_GENRE_COUNT = 10   # genres with fewer songs → merged into 'other'


# ── Genre label setup ────────────────────────────────────────────────────────

def build_genre_vocab(genre_json):
    """Return (song_id→coarse_genre, genre→int, int→genre, class_weights) after merging rare genres."""
    ...
    # Inverse-frequency class weights for balanced genre loss
...
def attach_genre_labels(graphs, song2genre):
    """Set g.genre_label (int) on each graph; -1 = unknown."""
    for g in graphs:
...
def train_multitask(model, train_graphs, val_graphs, genre2id, class_weights):
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for start in range(0, len(train_graphs), BATCH_SIZE):
            ...
            # Build occ→song-in-batch index (PyG Batch sets batch['occ'].batch)
            occ_batch = batch['occ'].batch  # [N_occ]
            ...
            # ── Chord loss (occ-level) ──
            labels = batch['occ'].y
            mask   = labels != -100
            chord_loss = F.cross_entropy(chord_logits[mask], labels[mask])

            # ── Genre loss (song-level, masked to known labels) ──
        ...
        if val_metrics['cross_entropy'] < best_val_loss:
            ...
            torch.save(model.state_dict(), f'{RESULTS_DIR}/multitask_best.pt')
...
def eval_genre(model, graphs, batch_size=32):
    """Top-1 accuracy for genre prediction."""
    model.eval()
    hits = 0
    total = 0
...
@torch.no_grad()
def collect_song_embeddings(model, graphs, batch_size=32):
    """Return (song_embeddings [N, H], genre_labels [N], song_ids [N])."""
...
def plot_training_curves(history, save_path):
    ...
    axes[0].set_title('Chord cross-entropy')
...
def plot_genre_umap(embeddings, genre_labels, id2genre, save_path):
    ...
    ax.set_title('UMAP — multi-task GNN song embeddings (colored by genre)')
...
def main():
    ...
    print(f'Genre distribution: {Counter(song2genre.values())}')
    print(f'Class weights: { {id2genre[i]: f"{w:.2f}" for i, w in enumerate(class_weights.tolist())} }')
    ...
    # Model — same causal v2 config + genre head
    ...
    print(f'Multi-task model parameters: {n_params:,}')

    # Train
    print(f'\n=== Training multi-task GNN (genre_weight={GENRE_WEIGHT}, balanced loss) ===')
    history = train_multitask(model, train_g, val_g, genre2id, class_weights)

    # Load best and evaluate
    model.load_state_dict(torch.load(f'{RESULTS_DIR}/multitask_best.pt', map_location=DEVICE))
    ...
    # Save
    with open(f'{RESULTS_DIR}/multitask_history.json', 'w') as f:
        json.dump(history, f, indent=2)
    with open(f'{RESULTS_DIR}/multitask_genre_acc.json', 'w') as f:
        json.dump({
    ...
    plot_training_curves(history, f'{RESULTS_DIR}/multitask_training_curves.png')

    # UMAP of new embeddings
    ...
    plot_genre_umap(embs, genre_labels, id2genre, f'{RESULTS_DIR}/multitask_genre_umap.png')
